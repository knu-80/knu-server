...
public interface NoticeQueryControllerDocs {
    ...
    @Operation(summary = "공지사항 목록 조회", description = "커서 기반 페이징으로 공지사항 목록을 조회합니다. 탈퇴한 작성자의 공지는 목록에서 제외됩니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공")
    })
    ResponseEntity<kr.co.knuserver.global.exception.ApiResponse<List<NoticeListResponse>>> getNotices(
            @Parameter(description = "마지막으로 조회한 공지사항 ID. 첫 페이지 요청 시 생략")
            Long lastId,
            @Parameter(description = "한 번에 조회할 개수 (기본값: 10)")
            int size
    );
